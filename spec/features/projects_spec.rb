...
feature "Projects" do
  ...
  describe "Multiple users interaction" do    
    ...
    scenario "User forks other users projects" do
      ...
      expect(find('.project')).to have_content('test2 / public_project from test1 / public_project')
      click_link "logout"
      sign_up_with("t3@test.com","test3","secret12345")
      visit "/test2/public_project"
      click_link "Fork"
      expect(page.current_path).to eq("/test3/public_project")
      expect(find('.project')).to have_content('test3 / public_project from test2 / public_project')
